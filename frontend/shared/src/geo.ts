/* Coğrafi saf yardımcılar — web ve mobil AYNI aritmetiği okur (M-7'de shared'a taşındı).
   Backend `GeoMath` ile birebir: iki istemcinin merkezi/mesafesi ayrışamaz. */
import type { ParticipantDto, SessionView } from "./api";
...
/** Orta noktaya EN YAKIN katılımcı — "orta nokta {{isim}} tarafında" notunun kaynağı (§4.5b:
    orta nokta hıza ters ağırlıklı kaydığı için kimin tarafında olduğu anlamlı). */
