  ...
  return (
    <li className="flex items-center gap-3 px-4 py-3">
      <span className="flex min-w-0 flex-1 flex-col gap-0.5">
        {props.href ? (
          <a href={props.href} target="_blank" rel="noreferrer" className="text-[0.875rem] font-bold text-ink">{props.label}</a>
        ) : (
          <span className="text-[0.875rem] font-bold text-ink">{props.label}</span>
        )}
