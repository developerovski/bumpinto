...
type State = {
  ...
  /** Hesaba bağlı: çıkışta sıfırlanır (`authStore`), sonraki hesaba süzgeç ve konum sızmaz. */
  reset: () => void;
};
...
/**
 * Keşfet listesi (M-11). Web `discoverStore` ile AYNI şekil; aralık süzgeci ve sıralama kuralları
 * `@bumpinto/shared` `planRange.ts`'te — burada yalnız durum ve istek.
 */
